...
class SendingToPostgres(ExtractedInformationStorage):
    """docstring forSendingToPostgres.ExtractedInformationStorageef __init__(self, arg):
        superSendingToPostgres,ExtractedInformationStorage.__init__()
        self.arg = arg"""
    def process_storedata(self,item,spider):
        conn = psycopg2.connect("dbname='news_please' user='postgres' host='localhost' password='welcome' port='5432'")
        cursor = conn.cursor()

        try:

            cmmd = "INSERT INTO scrappednews(source,scrapetime,hyperlink,heading,article,author,imageurl,publishtime) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)"
            cursor.execute(cmmd,(ExtractedInformationStorage.item['url'],ExtractedInformationStorage.datestring_to_date(item['date_download']),ExtractedInformationStorage.item['url'],ExtractedInformationStorage.item['title'],ExtractedInformationStorage.item['title_page'],ExtractedInformationStorage.item['authors'],ExtractedInformationStorage.item['image_url'],ExtractedInformationStorage.datestring_to_date(item['date_publish'])))
            conn.commit()
            cursor.close()
...
STP = SendingToPostgres()
STP.process_storedata()
